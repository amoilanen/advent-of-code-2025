#!/usr/bin/env python3
"""Advent of Code 2025 - Main Entry Point

Usage:
    python aoc2025.py           # Run all days
    python aoc2025.py 1         # Run day 1
    python aoc2025.py day01     # Run day 1
"""

import sys
from internal.days import day01, day02, day03, day04, day05, day06, day07, day08, day09, day10, day11, day12


def main():
    if len(sys.argv) > 1:
        run_specific_day(sys.argv[1])
    else:
        run_all_days()


def run_all_days():
    print("Advent of Code 2025 - Solutions")
    print("=" * 40)
    print()

    run_day01()
    run_day02()
    run_day03()
    run_day04()
    run_day05()
    run_day06()
    run_day07()
    run_day08()
    run_day09()
    run_day10()
    run_day11()
    run_day12()


def run_specific_day(day: str):
    """Run a specific day's solution."""
    day_map = {
        '1': run_day01,
        '2': run_day02,
        '3': run_day03,
        '4': run_day04,
        '5': run_day05,
        '6': run_day06,
        '7': run_day07,
        '8': run_day08,
        '9': run_day09,
        '10': run_day10,
        '11': run_day11,
        '12': run_day12
    }

    runner = day_map.get(day.lower())
    if runner:
        runner()
    else:
        print(f"Unknown day: {day}", file=sys.stderr)
        print("Usage: python aoc2025.py [day]", file=sys.stderr)
        print("Example: python aoc2025.py 1", file=sys.stderr)
        sys.exit(1)


def run_day01():
    """Run Day 1 solution."""
    print("Day 1:")
    parsed = day01.parse(day01.DAY_INPUT)
    print(f"  Part 1: {day01.part1(parsed)}")
    print(f"  Part 2: {day01.part2(parsed)}")
    print()


def run_day02():
    """Run Day 2 solution."""
    print("Day 2:")
    parsed = day02.parse(day02.DAY_INPUT)
    print(f"  Part 1: {day02.part1(parsed)}")
    print(f"  Part 2: {day02.part2(parsed)}")
    print()


def run_day03():
    """Run Day 3 solution."""
    print("Day 3:")
    parsed = day03.parse(day03.DAY_INPUT)
    print(f"  Part 1: {day03.part1(parsed)}")
    print(f"  Part 2: {day03.part2(parsed)}")
    print()


def run_day04():
    """Run Day 4 solution."""
    print("Day 4:")
    parsed = day04.parse(day04.DAY_INPUT)
    print(f"  Part 1: {day04.part1(parsed)}")
    print(f"  Part 2: {day04.part2(parsed)}")
    print()

def run_day05():
    """Run Day 5 solution."""
    print("Day 5:")
    parsed = day05.parse(day05.DAY_INPUT)
    print(f"  Part 1: {day05.part1(parsed)}")
    print(f"  Part 2: {day05.part2(parsed)}")
    print()


def run_day06():
    """Run Day 6 solution."""
    print("Day 6:")
    parsed_part1 = day06.parse_part1(day06.DAY_INPUT)
    parsed_part2 = day06.parse_part2(day06.DAY_INPUT)
    print(f"  Part 1: {day06.part1(parsed_part1)}")
    print(f"  Part 2: {day06.part2(parsed_part2)}")
    print()

def run_day07():
    """Run Day 7 solution."""
    print("Day 7:")
    grid, start_row, start_col = day07.parse(day07.DAY_INPUT)
    print(f"  Part 1: {day07.part1(grid, start_row, start_col)}")
    print(f"  Part 2: {day07.part2(grid, start_row, start_col)}")
    print()

def run_day08():
    """Run Day 8 solution."""
    print("Day 8:")
    boxes = day08.parse(day08.DAY_INPUT)
    print(f"  Part 1: {day08.part1(boxes)}")
    print(f"  Part 2: {day08.part2(boxes)}")
    print()

def run_day09():
    """Run Day 9 solution."""
    print("Day 9:")
    tiles = day09.parse(day09.DAY_INPUT)
    print(f"  Part 1: {day09.part1(tiles)}")
    print(f"  Part 2: {day09.part2(tiles)}")
    print()

def run_day10():
    """Run Day 10 solution."""
    print("Day 10:")
    machines_part1 = day10.parse(day10.DAY_INPUT)
    machines_part2 = day10.parse_part2(day10.DAY_INPUT)
    print(f"  Part 1: {day10.part1(machines_part1)}")
    print(f"  Part 2: {day10.part2(machines_part2)}")
    print()

def run_day11():
    """Run Day 11 solution."""
    print("Day 11:")
    graph = day11.parse(day11.DAY_INPUT)
    print(f"  Part 1: {day11.part1(graph)}")
    print(f"  Part 2: {day11.part2(graph)}")
    print()

def run_day12():
    """Run Day 12 solution."""
    print("Day 12:")
    data = day12.parse(day12.DAY_INPUT)
    print(f"  Part 1: {day12.part1(data)}")
    print()


if __name__ == "__main__":
    main()
